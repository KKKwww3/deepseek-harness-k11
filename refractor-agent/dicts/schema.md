...
**唯一维护文件：`dicts/refractions.yml`**。它是构建源：`embed.py` 会生成向量表 `refractor_types` 和命名表 `refraction_names`。改完跑 `python scripts/embed.py` 自动全量重建（自动校验，报错会说明哪行不对）。
...
- pattern: 碎冰            # 图案类型（新增即自动成为 VLM 枚举，无需单独维护枚举文件）
  ...
  names:                   # 对外叫法，按品牌×系列（写入 refraction_names）
...
| `names` | dict | 推荐 | key=`品牌-系列`，value=`{name, name_en}`；embed.py 展开写入数据库 `refraction_names` |
...
> 识别稳定返回，运行时从数据库 `refraction_names` 查询；员工只需保证 `names` 的 key
> 与 VLM 返回的 brand/series 一致（脚本会小写 + 去空白归一化）。修改后重新运行 `embed.py`。
